...
const EditableCell: React.FC<{ value: number; onChange: (newValue: number) => void }> = ({ value, onChange }) => {
    ...
    return (
        <input
            ...
            className="w-full bg-transparent text-[var(--dark-gray)] font-mono p-1 rounded border-transparent focus:bg-white focus:border-[var(--dark-gray)] focus:ring-0 transition"
        />
    );
}
...
const NodesTable: React.FC<NodesTableProps> = ({ nodes, onUpdateNode, selectedElement, onDeleteNode }) => {
  return (
    <div className="bg-white rounded-lg shadow-lg p-4 h-full flex flex-col border border-[var(--light-blue)]">
      <h2 className="text-xl font-bold text-[var(--dark-gray)] mb-4">Nodes (mm)</h2>
        ...
        <table className="w-full text-left text-sm">
          <thead className="sticky top-0 bg-white border-b-2 border-[var(--light-blue)]">
            <tr>
              ...
              <th className="p-2 text-[var(--medium-gray)] font-semibold">X</th>
              <th className="p-2 text-[var(--medium-gray)] font-semibold">Y</th>
          ...
          <tbody>
            {nodes.map((node, index) => {
              ...
              return (
              <tr key={node.id} className={`border-t border-[var(--light-blue)] transition-colors ${isSelected ? 'bg-[var(--light-blue)]' : 'hover:bg-[#f7fcfc]'}`}>
                <td className="p-2 font-mono text-[var(--dark-gray)]">{index + 1}</td>
                <td className="p-1">
                    <EditableCell 
                        value={node.x}
             ...
             {nodes.length === 0 && (
                <tr>
                    <td colSpan={4} className="text-center p-4 text-[var(--medium-gray)]">No nodes added yet.</td>
                </tr>
            )}
          </tbody>
        </table>
